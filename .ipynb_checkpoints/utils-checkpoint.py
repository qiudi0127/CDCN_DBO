...
def performances_Cele(map_score_val_filename):
    # val 
    with open(map_score_val_filename, 'r') as file:
        lines = file.readlines()
    val_scores = []
    ...
    return val_threshold, val_ACC, val_ACER,val_APCER,val_BPCER
...
def get_face_box(file_name,h,w,is_square=False):
    if not os.path.exists(file_name):
        return None
    with open(file_name,'r') as f:
        left_tp = f.readline()
        right_bt = f.readline()
    sps = left_tp.strip().split(' ')
    x0 = sps[0].split(':')[1]
    y0 = sps[1].split(':')[1]

    sps = right_bt.strip().split(' ')
    x1 = sps[0].split(':')[1]
    y1 = sps[1].split(':')[1]
    ...
    if is_square:
        h_ = y_max - y_min + 1
        w_ = x_max - x_min + 1
        max_side = np.maximum(h_, w_)
        x_min = max( 0 , int( x_min + w_ * 0.5 - max_side * 0.5) )
        y_min = max( 0 , int(y_min + h_ * 0.5 - max_side * 0.5) )
        x_max = min( w, int(x_min + max_side - 1) )
        y_max = min( h, int(y_min + max_side - 1) )
